...
local tick = tick and tick or os.clock and os.clock
local next = next
local ipairs = ipairs
...
function M.adjust(t, key, f)
	if t[key] == nil then
		error("key not existing in table", 2)
	end
...
function M.sameKeys(tA, tB)
	for key in next, tA do
		if not tB[key] then
			return false
		end
	end

	for key in next, tB do
		if not tA[key] then
			return false
		end
	end
...
function M.shift(array, n)
	n = math.min(n or 1, #array)
	local ret = {}
	local length = 0

	for _ = 1, n do
		length = length + 1
		ret[length] = table.remove(array, 1)
	end

	return table.unpack(ret, 1, length)
end
...
function M.last(array, n)
	local l = #array
	n = n and l - math.min(n - 1, l - 1) or 2
...
function M.flatten(array, shallow)
	...
	for _, value in ipairs(array) do
		if type(value) == "table" then
			...
			for _, item in ipairs(new_flattened) do
				length = length + 1
				_flat[length] = item
			end
		else
...
function M.intersection(...)
	local arg = {...}
	local array = table.remove(arg, 1)
	local _intersect = {}
	local length = 0
...
function M.zipWith(f, ...)
	...
	local _ans = {}
	for i = 1, n do
		_ans[i] = f(table.unpack(M.pluck(args, i)))
	end
...
function M.range(from, to, step)
	...
	local _ranged = {from}
	local steps = math.max(math.floor((to - from) / step), 0)
	for i = 1, steps do
...
function M.powerset(array)
	...
	for _, v in ipairs(array) do
		for j = 1, length do--#powerset do
			local set = powerset[j]
			length = length + 1
			powerset[length] = M.push(M.slice(set), v)
		end
...
function M.aperture(array, n)
	...
	return coroutine.wrap(function()
		partgen3(array, n or 2, coroutine.yield)
	end)
end
...
function M.concat(array, sep, i, j)
	return table.concat(M.map(array, tostring), sep, i, j)
end
...
local KEY_VALUE_WEAK_TABLE = {__mode = "kv"}
...
function M.memoize(f)
	local _cache = setmetatable({}, KEY_VALUE_WEAK_TABLE)
	return function(key)
...
function M.bindn(f, ...)
	local args = {...}
	return function(...)
		return f(table.unpack(M.append(args, {...})))
	end
end
...
function M.uniqueId(template)
	...
	if template then
		if type(template) == "string" then
			return string.format(template, unique_id_counter)
...
function M.flip(f)
	return function(...)
		return f(table.unpack(M.reverse{...}))
	end
end
...
function M.ary(f, n)
	...
	return function(...)
		...
		return f(table.unpack(fargs, 1, n))
	end
end
...
function M.overArgs(f,...)
	...
	return function(...)
		...
		return f(table.unpack(_args, 1, _args.n))
	end
end
...
function M.partial(f,...)
	...
	return function(...)
		...
		return f(table.unpack(M.append(f_args, n_args)))
	end
end
...
function M.partialRight(f,...)
	...
	return function(...)
		...
		return f(table.unpack(M.append(n_args, f_args)))
	end
end
...
function M.curry(f, n_args)
	...
	local function scurry(v)
		...
		if #_args < n_args then
			...
			local r = table.pack(f(table.unpack(_args)))
			_args = {}
			return table.unpack(r, 1, r.n)
		end
	end
...
function M.time(f, ...)
	local stime = tick()
	local r = table.pack(f(...))
	return tick() - stime, table.unpack(r, 1, r.n)
end
...
function M.functions(obj, recurseMt)
	...
	if recurseMt then
		...
		if mt and mt.__index then
			...
			for _, fn in ipairs(mt_methods) do
				length = length + 1
				_methods[length] = fn
			end
		end
...
function M.pick(obj, ...)
	...
	local _picked = {}
	for _, property in next, whitelist do
...
do
	...
	function f.import(context, noConflict)
		context = context or shared or _ENV or _G
