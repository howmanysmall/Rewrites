--[[
	...
	Originally by evaera, this version has been changed a little to match the style of the rest of my code as well as speeding things up slightly.
]]
...
function Promise._newWithSelf(executor, ...)
	...
	executor(promise, table.unpack(args, 1, args.n))
...
local function promisify(callback, values, length, resolve, reject, traceback)
	local ok, resultLength, resultValues = packResult(pcall(callback, table.unpack(values, 1, length)))
	if ok then
		resolve(table.unpack(resultValues, 1, resultLength))
	else
...
function Promise.promisify(callback)
	return function(...)
		...
		return Promise.new(function(resolve, reject)
			local thread = coroutine.create(promisify)
			coroutine.resume(thread, callback, values, length, resolve, reject, traceback)
		end)
	end
end
...
function Promise.prototype:await(...)
	...
	return status == Promise.Status.Resolved, table.unpack(result, 1, length - 1)
end
...
function Promise.prototype:expect(...)
	...
	return table.unpack(result, 1, length - 1)
end
...
local function _reject(self, err)
...
function Promise.prototype:_reject(...)
	...
	if not isEmpty(self._queuedReject) then
		...
		local err = tostring((...))
		local thread = coroutine.create(_reject)
