...
-- This version is speedier with this BindableEvent library.
...
local function MandateType(value, type, paramName, nullable)
	...
	assert(typeof(value) == type, string.format(ERR_INVALID_TYPE, paramName or "ERR_NO_PARAM_NAME", type, typeof(value)))
end
...
local function GetPositionAtTime(time, origin, initialVelocity, acceleration)
	...
	local force = Vector3.new((acceleration.X * timeSquared) / 2,(acceleration.Y * timeSquared) / 2, (acceleration.Z * timeSquared) / 2)
...
local function BaseFireMethod(self, origin, directionWithMagnitude, velocity, cosmeticBulletObject, ignoreDescendantsInstance, ignoreWater, bulletAcceleration, list, isWhitelist)
	...
	assert(typeof(velocity) == "Vector3" or type(velocity) == "number", string.format(ERR_INVALID_TYPE, "velocity", "Variant<Vector3, number>", typeof(velocity))) -- This one's an odd one out.
...
function FastCast.new()
	return setmetatable({
		LengthChanged = BindableEvent.new(),
		RayHit = BindableEvent.new()
	}, FastCast)
end
...
function FastCast:Fire(origin, directionWithMagnitude, velocity, cosmeticBulletObject, ignoreDescendantsInstance, ignoreWater, bulletAcceleration)
	assert(getmetatable(self) == FastCast, string.format(ERR_NOT_INSTANCE, "Fire", "FastCast.new()"))
...
function FastCast:FireWithWhitelist(origin, directionWithMagnitude, velocity, whitelist, cosmeticBulletObject, ignoreWater, bulletAcceleration)
	assert(getmetatable(self) == FastCast, string.format(ERR_NOT_INSTANCE, "FireWithWhitelist", "FastCast.new()"))
...
function FastCast:FireWithBlacklist(origin, directionWithMagnitude, velocity, blacklist, cosmeticBulletObject, ignoreWater, bulletAcceleration)
	assert(getmetatable(self) == FastCast, string.format(ERR_NOT_INSTANCE, "FireWithBlacklist", "FastCast.new()"))
